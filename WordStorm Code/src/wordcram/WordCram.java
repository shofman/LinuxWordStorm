 ...
 * the processing functionality, but are serializable. Getters and setters have been added to the engine
 */
 ...
 * {@link Word#setColor(int)}, {@link Word#setFont(MyPFont)}, {@link
 ...
 * also take MyPFonts), or {@link #withFonter(WordFonter)}
 * 
...
/*
 * @author Scott Hofman
 * Added a class to find the class based on pixels of final image, which are changed by the scaling factor done on display of wordstorms.
 * Also made serializable.
 */
public class WordCram implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4819847854774323487L;
	...
	private Word[] words;
	private Word[] unique;
	private transient TextSource textSource;
	private transient String extraStopWords = "";
	...
	private transient PGraphics destination = null;
	 ...
	 */
	public WordCram(MyPApplet parent) {
		this.parent = parent;
	}
	...
	public WordCram fromHtmlString(String... html) {
		return fromText(new Html(MyPApplet.join(html, "")));
	}
	...
	public WordCram fromTextString(String... text) {
		return fromText(new Text(MyPApplet.join(text, "")));
	}
	 ...
	 * <a href="http://processing.org/reference/MyPFont.html" target="blank">MyPFont</a>
	 * for each fontName, via
	 * <a href="http://processing.org/reference/createFont_.html" target="blank">createFont</a>,
	 * and will render words in one of those MyPFonts.
	 * 
	 * @return The WordCram, for further setup or drawing.
	 */
	public WordCram withFonts(String... fontNames) {
		MyPFont[] fonts = new MyPFont[fontNames.length];
	...
	public WordCram withFont(String fontName) {
		MyPFont font = parent.createFont(fontName, 1);
		return withFont(font);
	}
	
	/**
	 * This WordCram will render words in one of the given 
	 * <a href="http://processing.org/reference/MyPFont.html" target="blank">MyPFonts</a>.
	 * 
	 * @return The WordCram, for further setup or drawing.
	 */
	public WordCram withFonts(MyPFont... fonts) {
	 ...
	 * <a href="http://processing.org/reference/MyPFont.html" target="blank">MyPFont</a>.
	 * 
	 * @param font the MyPFont to render the words in.
	 * @return The WordCram, for further setup or drawing.
	 */
	public WordCram withFont(MyPFont font) {
...
//			if(force && false) w.weight = 0.5f; // QUIM
	...
	public void setParent(MyPApplet parent, StormConf sc) {
	...
	public Word getRenderedWordAt(float x, float y) {
		Word rWord = getWordCramEngine().renderedWordAt(x, y);
		forgetEngine();
		return rWord;
	}
	...
	public int getIndex(Word w) {
		return java.util.Arrays.asList(words).indexOf(w);
	}
