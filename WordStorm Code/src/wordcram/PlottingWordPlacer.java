 ...
 * finally rendered.
 */
public class PlottingWordPlacer implements WordPlacer, Serializable {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = -6574480830980800430L;
	private transient MyPApplet parent;
	private WordPlacer wrappedPlacer;
	
	public PlottingWordPlacer(MyPApplet _parent, WordPlacer _wrappedPlacer) {
		parent = _parent;
	...
	public MyPVector place(Word word, int wordIndex, int wordsCount, int wordImageWidth, int wordImageHeight, int fieldWidth, int fieldHeight) {
		MyPVector v = wrappedPlacer.place(word, wordIndex, wordsCount, wordImageWidth, wordImageHeight, fieldWidth, fieldHeight);
		parent.pushStyle();
		parent.noFill();
