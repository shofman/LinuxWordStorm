 ...
 * the processing functionality, but are serializable. Added new bounding box identifier of words
 */
import java.awt.*;
...
class WordCramEngine implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 2683447324659964765L;
	...
	void moveTest(EngineWord eWord) {
		moveWordTest(eWord);
	}
	...
	void show(Map<String, CoordProp> index, int cloudInd) {
		for(EngineWord eWord: eWords){
			MyPVector loc = index.get(eWord.word.word).getCurrentLocation(cloudInd);
	...
	private void moveWord(EngineWord eWord){
		...
		MyPVector loc = eWord.getCurrentLocation();
...
//		}	
	...
	private boolean moveWordTest(EngineWord eWord) {
		Word word = eWord.word;
		Rectangle2D rect = eWord.getShape().getBounds2D(); // TODO can we move these into EngineWord.setDesiredLocation? Does that make sense?		
		int wordImageWidth = (int)rect.getWidth();
		int wordImageHeight = (int)rect.getHeight();
	...
	void placeUnique(Map<String, CoordProp> index, Word[] unique, long rSeed){
		...
		for(EngineWord eWord: eWords){
			MyPVector loc = index.get(eWord.word.word).getCurrentLocation(eWord.word.cloudIndex);
			eWord.setLocations(loc, loc);
			eWord.finalizeLocation();
		}
...
//			}
	...
	Word renderedWordAt(float x, float y) {
		for (int i = 0; i < eWords.length; i++) {
			if (eWords[i].wasPlaced()) {
				Word w = eWords[i].word;
				float renderedX = w.getRenderedPlace().x;
				float renderedY = w.getRenderedPlace().y;
				//check if within bounding box of top left corner and width/height of box
				if (x >= renderedX && x <= renderedX + eWords[i].word.getBounds().getWidth()) {
					if (y >= renderedY && y <= renderedY + eWords[i].word.getBounds().getHeight()) {
						return w;
					}
				}
			}
