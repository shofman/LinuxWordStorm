 ...
 * <p>
 * WordCram gets a MyPVector from the nudger, and adds it to the word's desired
 * location, to find the next spot to try fitting the word. Note that the
 * MyPVectors returned from a nudger <i><b>don't accumulate</b></i>: if the placer
 ...
 * scale the MyPVector, since the nudges don't accumulate (see above).
 * 
 ...
 * @author Dan Bernier
 */
public interface WordNudger extends Serializable {
	 ...
	 * @return the MyPVector to add to the word's desired location, to get the
	 *         next spot to try fitting the word
	 */
	public MyPVector nudgeFor(Word word, int attemptNumber);
}
