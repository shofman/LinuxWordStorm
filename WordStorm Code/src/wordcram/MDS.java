...
public class MDS implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6750019151914466381L;
	...
	public MDS(WordStorm ws, AppletConf apConf, StormConf conf){
		...
		this.compEps = 1;
		this.sameEps = 1;
		this.center = new MyPVector(apConf.getWidth()*1.0f/2,apConf.getHeight()*1.0f/2);
	...
	public float disPositions(Word[] wii, Word[] wjj){
		...
		while(ii!=wii.length && jj!=wjj.length){
			if(wii[ii].word.compareTo(wjj[jj].word) > 0){
				...
				MyPVector iiLoc = ws.index.get(wii[ii].word).getCurrentLocation(wii[ii].cloudIndex);
				MyPVector jjLoc = ws.index.get(wjj[jj].word).getCurrentLocation(wjj[jj].cloudIndex);
				dis = MyPVector.dist(iiLoc,jjLoc);
				sum += dis*dis;
				++ii; ++jj;
			}else {
				System.err.println("error no pensat");
	...
	public Rectangle2D rectangleLoc(String word, int cloudIndex){
		MyPVector wordLoc = ws.index.get(word).getCurrentLocation(cloudIndex);
	...
	public MyPVector minCtAxisOverlapDirection(Rectangle2D a, Rectangle2D b){
		...
		MyPVector dir = new MyPVector();
		double min = Double.MAX_VALUE;
		if( min > west){
			min = west;
			dir = new MyPVector(1,0);
		}
		if( min > east){
			min = east;
			dir = new MyPVector(-1,0);
...
//		dir.mult(0.1f);
	...
	public MyPVector minCtOverlapDirection(Rectangle2D a, Rectangle2D b){
		double west = b.getX() + b.getWidth() - a.getX();
		double east = a.getX() + a.getWidth() - b.getX();
		double north = b.getY() + b.getHeight() - a.getY();
		double south = a.getY() + a.getHeight() - b.getY();
	...
	public MyPVector minAxisOverlapDirection(Rectangle2D a, Rectangle2D b){
		double west = b.getX() + b.getWidth() - a.getX();
		double east = a.getX() + a.getWidth() - b.getX();
		double north = b.getY() + b.getHeight() - a.getY();
		double south = a.getY() + a.getHeight() - b.getY();
		...
		if( min > west){
			min = west;
			dir = new MyPVector((float) Math.max(west,step),0);
		}
		if( min > east){
			min = east;
			dir = new MyPVector((float)-Math.max(east,step),0);
		}
		if( min > north){
			min = north;
			dir = new MyPVector(0,(float)Math.max(north,step));
		}
		if( min > south){
			min = south;
			dir = new MyPVector(0,(float)-Math.max(south,step));
		}
		return dir;
	}
	
	public MyPVector minAxisOverlapDirectionNoise(Rectangle2D a, Rectangle2D b){
		double west = b.getX() + b.getWidth() - a.getX();
		double east = a.getX() + a.getWidth() - b.getX();
		double north = b.getY() + b.getHeight() - a.getY();
		double south = a.getY() + a.getHeight() - b.getY();
		...
		if( min > west){
			min = west;
			dir = new MyPVector((float) Math.max(west,step),(float)(eps*min));
		}
		if( min > east){
			min = east;
			dir = new MyPVector((float)-Math.max(east,step),(float)(eps*min));
		}
		if( min > north){
			min = north;
			dir = new MyPVector((float)(eps*min),(float)Math.max(north,step));
		}
		if( min > south){
			min = south;
			dir = new MyPVector((float)(eps*min),(float)-Math.max(south,step));
		}
		return dir;
	}
	
	
	public MyPVector minOverlapDirection(Rectangle2D a, Rectangle2D b){
		double west = b.getX() + b.getWidth() - a.getX();
		double east = a.getX() + a.getWidth() - b.getX();
		double north = b.getY() + b.getHeight() - a.getY();
		double south = a.getY() + a.getHeight() - b.getY();
	...
	public MyPVector minOverlapDirectionRed(Rectangle2D a, Rectangle2D b){
		double west = b.getX() + b.getWidth() - a.getX();
		double east = a.getX() + a.getWidth() - b.getX();
		double north = b.getY() + b.getHeight() - a.getY();
		double south = a.getY() + a.getHeight() - b.getY();
